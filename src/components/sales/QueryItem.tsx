...
const QueryItem: React.FC<QueryItemProps> = ({
  ...
  const submitResponseMutation = useMutation({
    ...
    onError: (error: Error) => {
      ...
      if (error.message.includes('Unauthorized')) {
        alert('❌ Unauthorized Access: You cannot respond to this query as it is not marked for the Sales team. Contact Operations if you believe this is an error.');
  ...
  const handleSubmitResponse = () => {
    ...
    const response: QueryResponse = {
      ...
      team: user?.role || 'Sales',
      respondedBy: user?.name || 'Sales Team Member',
      timestamp: new Date().toISOString()
    };
