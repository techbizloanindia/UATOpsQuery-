...
export function CreditQueryProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchQueries = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/queries?team=Credit&status=pending');
      ...
      if (data.success) {
        const formattedQueries = data.data.map((query: any) => ({
          ...
          markedForTeam: query.sendToCredit ? 'credit' : '',
          allowMessaging: query.sendToCredit,
          priority: 'medium',
          status: query.status,
