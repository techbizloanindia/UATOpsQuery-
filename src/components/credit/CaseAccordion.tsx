...
const CaseAccordion = ({ caseId, customerName, branch, queries: initialQueries }: CaseAccordionProps) => {
  ...
  const enabledQueries = queries.filter(q => q.allowMessaging && (q.markedForTeam === 'credit' || q.markedForTeam === 'both')).length;
      ...
      >
          <div className="flex items-center gap-2">
            ...
            <Link 
              ...
              className="font-bold text-lg text-green-600 tracking-wide hover:underline flex items-center gap-1"
            >
              {caseId}
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href={`/query-details/${caseId}`}
            className="px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center gap-1 text-sm mr-3"
          >
          ...
          {queries.length > 0 ? (
            <div className="p-4 space-y-3">
              ...
              {queries.map((query, index) => (
                <div key={query.id} className="border-l-4 border-green-200 pl-4">
